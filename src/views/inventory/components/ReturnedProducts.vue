                ...
                <table class="table">
                  <thead>
                    <tr>
                      ...
                      <th><i class="fas fa-user mr-1"></i> User Name</th>
...
export default {
  ...
  async mounted() {
    try {
      ...
      if (userData && userData.email) {
        ...
        await this.fetchReturnedProducts();
      }
    } catch (error) {
  ...
  computed: {
    filteredProducts() {
      ...
      return this.requests.filter((request) => {
        return (
          request.reqNo.toLowerCase().includes(keyword) ||
          ...
          request.quantity.toString().includes(keyword)
        );
      });
    },
  },
  ...
  methods: {
    async fetchReturnedProducts() {
      try {
        const response = await fetch("http://localhost:3000/getReturnedProducts");
        const data = await response.json();
        this.requests = data; // Storing fetched returned products
      } catch (error) {
