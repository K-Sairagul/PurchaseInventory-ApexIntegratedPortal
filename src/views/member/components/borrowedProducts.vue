            ...
            <div class="table-container">
              <table class="table">
                ...
                <tbody v-if="filteredTransfers.length > 0">
                  <template
                    v-for="(transfer, transferIndex) in filteredTransfers"
                    :key="transferIndex"
                  >
                    <tr
                      v-for="(product, prodIndex) in transfer.products"
                      :key="product._id"
                    >
                      <td v-if="prodIndex === 0" :rowspan="transfer.products.length">
                        {{ transfer.reqNo }}
                      </td>
                      <td>{{ product.productName }}</td>
                      <td>{{ product.quantityTransferred }}</td>
                      <td>
                        <span v-if="product.status === 1" class="returned-text"
                          >Returned</span
                        >
                        <button
                          v-else
                          class="return-btn"
                          @click="returnProduct(transfer, product)"
                        >
                          Return
                        </button>
                      </td>
                ...
                <tbody v-else>
                  <tr>
                    <td colspan="5">No transferred products found</td>
                  </tr>
                </tbody>
              </table>
            </div>
...
export default {
  name: "TransferProductMember",
  components: { SidebarMain, HeaderMain },
  data() {
    return {
      userProfile: {
        profilePhoto:
  ...
  computed: {
    filteredTransfers() {
      return this.transfers.filter((transfer) =>
        transfer.products.some((product) =>
  ...
  methods: {
    async fetchTransferredProducts() {
      try {
        ...
        const response = await fetch("http://localhost:3000/getTransferredProducts", {
          method: "POST",
    ...
    async returnProduct(transfer, product) {
      try {
        const userData = JSON.parse(sessionStorage.getItem("userData"));
        if (!userData || !userData.email) {
        ...
        const userInfoResponse = await fetch("http://localhost:3000/getUserInfo", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email: userData.email }),
        });
        ...
        if (!userInfo || !userInfo.username) {
          console.error("Failed to fetch user info from the backend");
          return;
        }

        // Log the data being sent
        console.log("Data being sent to backend:", {
          reqNo: transfer.reqNo,
          productId: product._id,
          productName: product.productName,
          quantity: product.quantityTransferred,
          email: transfer.email,
          userName: userInfo.username, // Use the username fetched from the backend
        });

        const response = await fetch("http://localhost:3000/returnProduct", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            reqNo: transfer.reqNo,
        ...
        if (result.success) {
          ...
          this.fetchTransferredProducts(); // Refresh the list after returning a product
        } else {
          console.error("Backend response error:", result.message);
        }
      } catch (error) {
    ...
    async fetchUserInfo() {
      try {
        ...
        if (data && data.username && data.role) {
          this.userName = data.username; // Set the username from the backend
          this.userRole = data.role;

          if (data.role !== "Member") {
            // Redirect to unauthorized page if the user is not a member
  ...
  mounted() {
    this.fetchUserInfo(); // Fetch user info (name and role) on component mount
    this.fetchTransferredProducts(); // Fetch transferred products on component mount
  },
};
</script>
